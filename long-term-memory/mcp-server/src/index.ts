...
server.tool(
  ...
  async ({ slug, category, context, title, body, tags }) => {
    ...
    if (!result.ok) return { content: [{ type: "text", text: "❌ Failed to store memory." }] };
