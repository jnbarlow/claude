...
// ─── AWS Secrets Manager ──────────────────────────────────────────
async function tryAwsSecretsManager(name) {
    try {
        ...
        console.error("[ltm-mcp] AWS Secrets Manager returned secret but no SecretString field.");
        return null;
    }
    catch (err) {
...
async function tryOnePasswordConnect(name) {
    try {
        ...
        const client = opModule.OnePasswordConnect({ serverURL: serverUrl, token });
        ...
        if (!connStr) {
            console.error(`[ltm-mcp] 1Password: Item "${name}" found but no connection_string field.`);
            return null;
        }
...
async function tryKeytar(name) {
    ...
    try {
        ...
        console.error(`[ltm-mcp] System keychain: No password found for service="${serviceName}", account="${name}".`);
        return null;
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        // Native binding failures are common on headless Linux without libsecret.
        console.error(`[ltm-mcp] System keychain (keytar) unavailable: ${msg}. ` +
 ...
 */
export async function resolveConnectionString(provider // 'aws' | '1password' | 'keychain' (default: keychain)
) {
    ...
    console.error(`[ltm-mcp] Could not resolve connection string from ${chosenProvider}. ` +
